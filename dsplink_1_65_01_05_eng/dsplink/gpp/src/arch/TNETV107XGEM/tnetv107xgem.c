 ...
 *  @leave  None.
 *
 *  @see    TNETV107XGEM_setup, TNETV107XGEM_exit
 *  ----------------------------------------------------------------------------
 */
NORMAL_API
DSP_STATUS
TNETV107XGEM_init (IN ProcessorId  dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   TNETV107XGEM_exit
 *
 ...
 *  @leave  None.
 *
 *  @see    TNETV107XGEM_init
 *  ----------------------------------------------------------------------------
 */
NORMAL_API
DSP_STATUS
TNETV107XGEM_exit (IN ProcessorId dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   TNETV107XGEM_start
 *
 ...
 *  @leave  None.
 *
 *  @see    TNETV107XGEM_stop
 *  ----------------------------------------------------------------------------
 */
NORMAL_API
DSP_STATUS
TNETV107XGEM_start (IN ProcessorId  dspId,
                  IN DSP_Object * dspState,
                  IN Uint32       dspAddr) ;


/** ----------------------------------------------------------------------------
 *  @func   TNETV107XGEM_stop
 *
 ...
 *  @leave  None.
 *
 *  @see    TNETV107XGEM_start
 *  ----------------------------------------------------------------------------
 */
NORMAL_API
DSP_STATUS
TNETV107XGEM_stop (IN ProcessorId dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   TNETV107XGEM_idle
 *
 *  @desc   Idles the DSP.
...
NORMAL_API
DSP_STATUS
TNETV107XGEM_idle (IN ProcessorId dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   TNETV107XGEM_intCtrl
 *
...
NORMAL_API
DSP_STATUS
TNETV107XGEM_intCtrl (IN         ProcessorId       dspId,
                   IN         DSP_Object     *  dspState,
                   IN         Uint32            intId,
...
NORMAL_API
DSP_STATUS
TNETV107XGEM_read (IN  ProcessorId  dspId,
                IN  DSP_Object * dspState,
                IN  Uint32       dspAddr,
...
NORMAL_API
DSP_STATUS
TNETV107XGEM_write (IN ProcessorId  dspId,
                 IN DSP_Object * dspState,
                 IN Uint32       dspAddr,
                 IN Endianism    endianInfo,
                 IN Uint32       numBytes,
...
NORMAL_API
Uint32
TNETV107XGEM_addrConvert (IN  ProcessorId        dspId,
                       IN  DSP_Object *       dspState,
                       IN  Uint32             addr,
...
NORMAL_API
DSP_STATUS
TNETV107XGEM_control (IN  ProcessorId  dspId,
                   IN  DSP_Object * dspState,
                   IN  Int32        cmd,
                       Pvoid        arg) ;
...
/** ============================================================================
 *  @name   TNETV107XGEM_Interface
 *
 *  @desc   The interface table exposed for the DSP component.
 *  ============================================================================
 */
DSP_Interface TNETV107XGEM_Interface = {
    &TNETV107XGEM_init,
    &TNETV107XGEM_exit,
    &TNETV107XGEM_start,
    &TNETV107XGEM_stop,
    &TNETV107XGEM_idle,
    &TNETV107XGEM_intCtrl,
    &TNETV107XGEM_read,
    &TNETV107XGEM_write,
    &TNETV107XGEM_addrConvert,
    &TNETV107XGEM_control,
} ;
...
TNETV107XGEM_init (IN ProcessorId  dspId, IN DSP_Object * dspState)
{
    ...
    TNETV107XGEM_HalObj *  halObj  = NULL   ;
    ...
    TRC_2ENTER ("TNETV107XGEM_init", dspId, dspState) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        if (     (dspObj->doDspCtrl != DSP_BootMode_Boot_Pwr)
            ...
            if (DSP_SUCCEEDED (status)) {
                status = TNETV107XGEM_halInit (&dspState->halObject, NULL) ;
                if (DSP_FAILED (status)) {
                    SET_FAILURE_REASON ;
                }
                else  {
                    halObj = (TNETV107XGEM_HalObj *) dspState->halObject ;
                }
            }
    ...
    TRC_1LEAVE ("TNETV107XGEM_init", status) ;
 ...
 *          2. Call TNETV107XGEM_idle () to let the DSP run in 'self loop'.
...
TNETV107XGEM_exit (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    LINKCFG_Dsp *       dspObj           ;
    LINKCFG_MemEntry *  memTable         ;
    TNETV107XGEM_HalObj *  halObj           ;
    LINKCFG_DspConfig * dspCfg           ;
    Uint32              i                ;

    TRC_2ENTER ("TNETV107XGEM_exit", dspId, dspState) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        if (dspState->halObject != NULL) {
            ...
            TNETV107XGEM_halExit (&dspState->halObject) ;
        }
    }

    TRC_1LEAVE ("TNETV107XGEM_exit", status) ;
                  ...
                  IN Uint32       dspAddr)
{
    ...
    TRC_3ENTER ("TNETV107XGEM_start", dspId, dspState, dspAddr) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        if (   (dspObj->doDspCtrl == DSP_BootMode_Boot_Pwr)
            ...
            gppAddr = TNETV107XGEM_addrConvert (
                                   dspId,
                                   dspState,
    ...
    TRC_1LEAVE ("TNETV107XGEM_start", status) ;
...
TNETV107XGEM_stop (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("TNETV107XGEM_stop", dspId, dspState) ;
    ...
    TRC_1LEAVE ("TNETV107XGEM_stop", status) ;
...
TNETV107XGEM_idle (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("TNETV107XGEM_idle", dspId, dspState) ;
    ...
    TRC_1LEAVE ("TNETV107XGEM_idle", status) ;
                   ...
                   IN OUT     Pvoid             arg)
{
    ...
    TRC_5ENTER ("TNETV107XGEM_intCtrl", dspId, dspState, intId, cmd, arg) ;
    ...
    TRC_1LEAVE ("TNETV107XGEM_intCtrl", status) ;
                 ...
                 OUT Uint8 *      buffer)
{
    ...
    TRC_1LEAVE ("TNETV107XGEM_read", status) ;
                  ...
                  IN Uint8 *       buffer)
{
    ...
    TRC_1LEAVE ("TNETV107XGEM_write", status) ;
                        ...
                        IN  DSP_AddrConvType   type)
{
    ...
    TRC_4ENTER ("TNETV107XGEM_addrConvert", dspId, dspState, addr, type) ;
    ...
    TRC_2PRINT (TRC_LEVEL1,
                "TNETV107XGEM_addrConvert addr [0x%x] convAddr [0x%x]\n",
                addr,
                convAddr) ;

    TRC_0LEAVE ("TNETV107XGEM_addrConvert") ;
                        ...
                        Pvoid       arg)
{
    ...
    TRC_4ENTER ("TNETV107XGEM_control", dspId, dspState, cmd, arg) ;

    DBC_Require (IS_VALID_PROCID (dspId)) ;

    status = DSP_ENOTIMPL ;

    TRC_1LEAVE ("TNETV107XGEM_control", status) ;

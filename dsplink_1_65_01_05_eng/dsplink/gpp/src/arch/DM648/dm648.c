 ...
 *
 *  @desc   Implementation of DSP interface-component for the DM648 arch.
 *
 *  @ver    1.65.01.05_eng
 *  ============================================================================
...
/** ============================================================================
 *  @const  DM648_BOOTROMENTRY_POINT
 *
 *  @desc   Entry point of BOOTROM CODE.
 *  ============================================================================
 */
#define DM648_BOOTROMENTRY_POINT  0x00800000
 ...
 *  @leave  None.
 *
 *  @see    DM648_setup, DM648_exit
 *  ----------------------------------------------------------------------------
 */
NORMAL_API
DSP_STATUS
DM648_init (IN ProcessorId  dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
DM648_exit (IN ProcessorId dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
DM648_start (IN ProcessorId  dspId,
...
NORMAL_API
DSP_STATUS
DM648_stop (IN ProcessorId dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
DM648_idle (IN ProcessorId dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
DM648_intCtrl (IN         ProcessorId       dspId,
               IN         DSP_Object     *  dspState,
               IN         Uint32            intId,
...
NORMAL_API
DSP_STATUS
DM648_read (IN  ProcessorId  dspId,
            IN DSP_Object *  dspState,
            IN  Uint32       dspAddr,
...
NORMAL_API
DSP_STATUS
DM648_write (IN ProcessorId  dspId,
             IN DSP_Object * dspState,
             IN Uint32       dspAddr,
             IN Endianism    endianInfo,
             IN Uint32       numBytes,
...
NORMAL_API
Uint32
DM648_addrConvert (IN  ProcessorId        dspId,
                   IN DSP_Object *        dspState,
                   IN  Uint32             addr,
...
NORMAL_API
DSP_STATUS
DM648_control (IN  ProcessorId dspId,
               IN DSP_Object * dspState,
               IN  Int32       cmd,
                   Pvoid       arg) ;
...
NORMAL_API
Bool
DM648_isRangeValid (IN  ProcessorId dspId, IN Uint32 dspAddr, IN Uint32 size) ;
...
DM648_init (IN ProcessorId  dspId, IN DSP_Object * dspState)
{
    ...
    Uint32              i         = 0       ;
    DM648_HalObj *     halObject = NULL    ;
    DM648_InitParams   initParams          ;
    LINKCFG_Dsp *       dspObj              ;
    LINKCFG_MemEntry *  memTable            ;
    LINKCFG_DspConfig * dspCfg ;

    TRC_2ENTER ("DM648_init", dspId, dspState) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        if (     (dspObj->doDspCtrl != DSP_BootMode_Boot_Pwr)
            ...
            status = DM648_halInit ((Pvoid *) &halObject, (Pvoid) &initParams);
            if (DSP_SUCCEEDED (status)) {
                dspState->halObject = halObject ;

         /*  ----------------------------------------------------------------
          *  In case DM648, when device is in pciboot mode. And when power is
          *  applied, GEM starts running with PLL bypass mode and starting PC
          *  value is ROM address (where bootrom code is written). This boot
          *  rom code programs the PLL to run at comparable frequency and then it
                ...
                if (   (dspObj->doDspCtrl == DSP_BootMode_Boot_Pwr)
                    ...
                    if (DSP_SUCCEEDED (status)) {
                        status = halObject->interface->bootCtrl (
                                            ...
                                            (Pvoid) DM648_BOOTROMENTRY_POINT);
                    }
                    else {
                        SET_FAILURE_REASON ;
                    }
...
DM648_exit (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("DM648_exit", dspId, dspState) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        halObject = (DM648_HalObj *)dspState->halObject ;
        dspCfg    = LDRV_LinkCfgPtr->dspConfigs [dspId] ;
        dspObj    = dspCfg->dspObject ;
        if (   (dspObj->doDspCtrl == DSP_BootMode_Boot_Pwr)
            || (dspObj->doDspCtrl == DSP_BootMode_NoLoad_NoPwr)
        ...
        DM648_halExit ((Pvoid *) &halObject) ;
    }

    TRC_1LEAVE ("DM648_exit", status) ;
...
NORMAL_API
DSP_STATUS
DM648_start (IN ProcessorId dspId, IN DSP_Object * dspState, IN Uint32 dspAddr)
{
    DSP_STATUS          status  = DSP_SOK    ;
    DM648_HalObj *      halObject = NULL    ;
    LINKCFG_DspConfig * dspCfg ;
    ...
    TRC_3ENTER ("DM648_start", dspId, dspState, dspAddr) ;
...
DM648_stop (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("DM648_stop", dspId, dspState) ;
...
DM648_idle (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("DM648_idle", dspId, dspState) ;
               ...
               IN OUT     Pvoid             arg)
{
    ...
    TRC_5ENTER ("DM648_intCtrl", dspId, dspState, intId, cmd, arg) ;
    ...
    TRC_1LEAVE ("DM648_intCtrl", status) ;
            ...
            OUT Uint8 *      buffer)
{
    ...
    else if (((DM648_HalObj *)dspState->halObject)->isMapActive == TRUE) {
        PRINT_Printf ("Do not why it is pending\n") ;
        status = DSP_EPENDING ;
        SET_FAILURE_REASON ;
    }
    else {
        halObject = (DM648_HalObj *)dspState->halObject ;
        if (DM648_isRangeValid (dspId, dspAddr, numBytes)) {
                   ...
                   IN  DSP_AddrConvType   type)
{
    ...
    TRC_4ENTER ("DM648_addrConvert", dspId, dspState, addr, type) ;
                   ...
                   Pvoid       arg)
{
    ...
    TRC_1LEAVE ("DM648_control", status) ;
...
DM648_isRangeValid (IN  ProcessorId dspId, IN Uint32 dspAddr, IN Uint32 size)
{
    ...
    TRC_3ENTER ("DM648_isRangeValid", dspId, dspAddr, size) ;
    ...
    TRC_1LEAVE ("DM648_isRangeValid", status) ;

 ...
 *  @leave  None.
 *
 *  @see    OMAP2530_setup, OMAP2530_exit
 *  ----------------------------------------------------------------------------
 */
NORMAL_API
DSP_STATUS
OMAP2530_init (IN ProcessorId  dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
OMAP2530_exit (IN ProcessorId dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
OMAP2530_start (IN ProcessorId  dspId,
...
NORMAL_API
DSP_STATUS
OMAP2530_stop (IN ProcessorId dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
OMAP2530_idle (IN ProcessorId dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   OMAP2530_intCtrl
 *
...
NORMAL_API
DSP_STATUS
OMAP2530_intCtrl (IN         ProcessorId       dspId,
                   IN         DSP_Object     *  dspState,
                   IN         Uint32            intId,
...
NORMAL_API
DSP_STATUS
OMAP2530_read (IN  ProcessorId  dspId,
                IN  DSP_Object * dspState,
                IN  Uint32       dspAddr,
...
NORMAL_API
DSP_STATUS
OMAP2530_write (IN ProcessorId  dspId,
                 IN DSP_Object * dspState,
                 IN Uint32       dspAddr,
                 IN Endianism    endianInfo,
                 IN Uint32       numBytes,
...
NORMAL_API
Uint32
OMAP2530_addrConvert (IN  ProcessorId        dspId,
                       IN  DSP_Object *       dspState,
                       IN  Uint32             addr,
...
NORMAL_API
DSP_STATUS
OMAP2530_control (IN  ProcessorId  dspId,
                   IN  DSP_Object * dspState,
                   IN  Int32        cmd,
                       Pvoid        arg) ;
...
/** ============================================================================
 *  @name   OMAP2530_Interface
 *
 *  @desc   The interface table exposed for the DSP component.
 *  ============================================================================
 */
DSP_Interface OMAP2530_Interface = {
    &OMAP2530_init,
    &OMAP2530_exit,
    &OMAP2530_start,
    &OMAP2530_stop,
...
OMAP2530_init (IN ProcessorId  dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("OMAP2530_init", dspId, dspState) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        if (     (dspObj->doDspCtrl != DSP_BootMode_Boot_Pwr)
            ...
            if (DSP_SUCCEEDED (status)) {
                status = OMAP2530_halInit (&dspState->halObject, NULL) ;
                if (DSP_FAILED (status)) {
                    SET_FAILURE_REASON ;
                }
                else  {
                    halObj = (OMAP2530_HalObj *) dspState->halObject ;
                    halObj->procId = dspId ;
                }
            }
    ...
    TRC_1LEAVE ("OMAP2530_init", status) ;
 ...
 *          2. Call OMAP2530_idle () to let the DSP run in 'self loop'.
...
OMAP2530_exit (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    LINKCFG_Dsp *       dspObj           ;
    LINKCFG_MemEntry *  memTable         ;
    OMAP2530_HalObj *  halObj           ;
    LINKCFG_DspConfig * dspCfg           ;
    Uint32              i                ;

    TRC_2ENTER ("OMAP2530_exit", dspId, dspState) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        if (dspState->halObject != NULL) {
            ...
            OMAP2530_halExit (&dspState->halObject) ;
        }
    }

    TRC_1LEAVE ("OMAP2530_exit", status) ;
                  ...
                  IN Uint32       dspAddr)
{
    ...
    TRC_3ENTER ("OMAP2530_start", dspId, dspState, dspAddr) ;
    ...
    TRC_1LEAVE ("OMAP2530_start", status) ;
...
OMAP2530_stop (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("OMAP2530_stop", dspId, dspState) ;
    ...
    TRC_1LEAVE ("OMAP2530_stop", status) ;
...
OMAP2530_idle (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("OMAP2530_idle", dspId, dspState) ;
    ...
    TRC_1LEAVE ("OMAP2530_idle", status) ;
                   ...
                   IN OUT     Pvoid             arg)
{
    ...
    TRC_5ENTER ("OMAP2530_intCtrl", dspId, dspState, intId, cmd, arg) ;
    ...
    TRC_1LEAVE ("OMAP2530_intCtrl", status) ;
                 ...
                 OUT Uint8 *      buffer)
{
    ...
    TRC_1LEAVE ("OMAP2530_read", status) ;
                  ...
                  IN Uint8 *       buffer)
{
    ...
    TRC_1LEAVE ("OMAP2530_write", status) ;
                        ...
                        IN  DSP_AddrConvType   type)
{
    ...
    TRC_4ENTER ("OMAP2530_addrConvert", dspId, dspState, addr, type) ;
    ...
    TRC_2PRINT (TRC_LEVEL1,
                "OMAP2530_addrConvert addr [0x%x] convAddr [0x%x]\n",
                addr,
                convAddr) ;

    TRC_0LEAVE ("OMAP2530_addrConvert") ;
                        ...
                        Pvoid       arg)
{
    ...
    TRC_4ENTER ("OMAP2530_control", dspId, dspState, cmd, arg) ;
    ...
    TRC_1LEAVE ("OMAP2530_control", status) ;

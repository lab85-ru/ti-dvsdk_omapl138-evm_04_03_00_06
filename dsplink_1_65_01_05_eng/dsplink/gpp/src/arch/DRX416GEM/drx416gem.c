 ...
 *
 *  @desc   Implementation of DRX416GEM sub-component.
 *
 *  @ver    1.65.01.05_eng
 *  ============================================================================
 ...
 *  @leave  None.
 *
 *  @see    DRX416GEM_setup, DRX416GEM_exit
 *  ----------------------------------------------------------------------------
 */
NORMAL_API
DSP_STATUS
DRX416GEM_init (IN ProcessorId  dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
DRX416GEM_exit (IN ProcessorId dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   DRX416GEM_start
 *
...
NORMAL_API
DSP_STATUS
DRX416GEM_start (IN ProcessorId  dspId,
...
NORMAL_API
DSP_STATUS
DRX416GEM_stop (IN ProcessorId dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
DRX416GEM_idle (IN ProcessorId dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   DRX416GEM_intCtrl
 *
...
NORMAL_API
DSP_STATUS
DRX416GEM_intCtrl (IN         ProcessorId       dspId,
                   IN         DSP_Object     *  dspState,
                   IN         Uint32            intId,
...
NORMAL_API
DSP_STATUS
DRX416GEM_read (IN  ProcessorId  dspId,
                IN  DSP_Object * dspState,
                IN  Uint32       dspAddr,
...
NORMAL_API
DSP_STATUS
DRX416GEM_write (IN ProcessorId  dspId,
                 IN DSP_Object * dspState,
                 IN Uint32       dspAddr,
                 IN Endianism    endianInfo,
                 IN Uint32       numBytes,
...
NORMAL_API
Uint32
DRX416GEM_addrConvert (IN  ProcessorId        dspId,
                       IN  DSP_Object *       dspState,
                       IN  Uint32             addr,
...
NORMAL_API
DSP_STATUS
DRX416GEM_control (IN  ProcessorId  dspId,
                   IN  DSP_Object * dspState,
                   IN  Int32        cmd,
                       Pvoid        arg) ;
...
/** ============================================================================
 *  @name   DRX416GEM_Interface
 *
 *  @desc   The interface table exposed for the DSP component.
 *  ============================================================================
 */
DSP_Interface DRX416GEM_Interface = {
    &DRX416GEM_init,
    &DRX416GEM_exit,
    &DRX416GEM_start,
    &DRX416GEM_stop,
...
DRX416GEM_init (IN ProcessorId  dspId, IN DSP_Object * dspState)
{
    ...
    MemMapInfo          mapInfo ;
    DRX416GEM_HalObj *  halObj  = NULL   ;
    LINKCFG_Dsp *       dspObj           ;
    LINKCFG_DspConfig * dspCfg ;
    LINKCFG_MemEntry *  memTable ;
    Uint32              i ;

    TRC_1ENTER ("DRX416GEM_init", dspId) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        if (     (dspObj->doDspCtrl != DSP_BootMode_Boot_Pwr)
            ...
            if (DSP_SUCCEEDED (status)) {
                status = DRX416GEM_halInit (&dspState->halObject, NULL) ;
                if (DSP_FAILED (status)) {
                    SET_FAILURE_REASON ;
                }
                else  {
                    halObj = (DRX416GEM_HalObj *) dspState->halObject ;
                }
            }
    ...
    TRC_1LEAVE ("DRX416GEM_init", status) ;
 ...
 *          2. Call DRX416GEM_idle () to let the DSP run in 'self loop'.
...
DRX416GEM_exit (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    LINKCFG_Dsp *       dspObj           ;
    LINKCFG_MemEntry *  memTable         ;
    DRX416GEM_HalObj *  halObj           ;
    LINKCFG_DspConfig * dspCfg           ;
    Uint32              i                ;

    TRC_1ENTER ("DRX416GEM_exit", dspId) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        DRX416GEM_halExit (&dspState->halObject) ;
    }

    TRC_1LEAVE ("DRX416GEM_exit", status) ;
                 ...
                 IN Uint32       dspAddr)
{
    ...
    TRC_2ENTER ("DRX416GEM_start", dspId, dspAddr) ;
    ...
    TRC_1LEAVE ("DRX416GEM_start", status) ;
...
DRX416GEM_stop (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("DRX416GEM_stop", dspId, dspState) ;
    ...
    TRC_1LEAVE ("DRX416GEM_stop", status) ;
...
DRX416GEM_idle (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("DRX416GEM_idle", dspId, dspState) ;
    ...
    TRC_1LEAVE ("DRX416GEM_idle", status) ;
                   ...
                   IN OUT     Pvoid             arg)
{
    ...
    TRC_5ENTER ("DRX416GEM_intCtrl", dspId, dspState, intId, cmd, arg) ;
    ...
    TRC_1LEAVE ("DRX416GEM_intCtrl", status) ;
                 ...
                 OUT Uint8 *      buffer)
{
    ...
    TRC_1LEAVE ("DRX416GEM_read", status) ;
                  ...
                  IN Uint8 *       buffer)
{
    ...
    TRC_1LEAVE ("DRX416GEM_write", status) ;
                        ...
                        IN  DSP_AddrConvType   type)
{
    ...
    TRC_4ENTER ("DRX416GEM_addrConvert", dspId, dspState, addr, type) ;
    ...
    TRC_2PRINT (TRC_LEVEL1,
                "DRX416GEM_addrConvert addr [0x%x] convAddr [0x%x]\n",
                addr,
                convAddr) ;

    TRC_0LEAVE ("DRX416GEM_addrConvert") ;
                        ...
                        Pvoid       arg)
{
    ...
    TRC_4ENTER ("DRX416GEM_control", dspId, dspState, cmd, arg) ;

    DBC_Require (IS_VALID_PROCID (dspId)) ;

    status = DSP_ENOTIMPL ;

    TRC_1LEAVE ("DRX416GEM_control", status) ;

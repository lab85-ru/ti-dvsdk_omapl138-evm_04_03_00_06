 ...
 *
 *  @desc   Implementation of DRA44XGEM sub-component.
 *
 *  @ver    1.65.01.05_eng
 *  ============================================================================
 ...
 *  @leave  None.
 *
 *  @see    DRA44XGEM_setup, DRA44XGEM_exit
 *  ----------------------------------------------------------------------------
 */
NORMAL_API
DSP_STATUS
DRA44XGEM_init (IN ProcessorId  dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
DRA44XGEM_exit (IN ProcessorId dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   DRA44XGEM_start
 *
...
NORMAL_API
DSP_STATUS
DRA44XGEM_start (IN ProcessorId  dspId,
...
NORMAL_API
DSP_STATUS
DRA44XGEM_stop (IN ProcessorId dspId, IN DSP_Object * dspState) ;
...
NORMAL_API
DSP_STATUS
DRA44XGEM_idle (IN ProcessorId dspId, IN DSP_Object * dspState) ;


/** ----------------------------------------------------------------------------
 *  @func   DRA44XGEM_intCtrl
 *
...
NORMAL_API
DSP_STATUS
DRA44XGEM_intCtrl (IN         ProcessorId       dspId,
                   IN         DSP_Object     *  dspState,
                   IN         Uint32            intId,
...
NORMAL_API
DSP_STATUS
DRA44XGEM_read (IN  ProcessorId  dspId,
                IN  DSP_Object * dspState,
                IN  Uint32       dspAddr,
...
NORMAL_API
DSP_STATUS
DRA44XGEM_write (IN ProcessorId  dspId,
                 IN DSP_Object * dspState,
                 IN Uint32       dspAddr,
                 IN Endianism    endianInfo,
                 IN Uint32       numBytes,
...
NORMAL_API
Uint32
DRA44XGEM_addrConvert (IN  ProcessorId        dspId,
                       IN  DSP_Object *       dspState,
                       IN  Uint32             addr,
...
NORMAL_API
DSP_STATUS
DRA44XGEM_control (IN  ProcessorId  dspId,
                   IN  DSP_Object * dspState,
                   IN  Int32        cmd,
                       Pvoid        arg) ;
...
/** ============================================================================
 *  @name   DRA44XGEM_Interface
 *
 *  @desc   The interface table exposed for the DSP component.
 *  ============================================================================
 */
DSP_Interface DRA44XGEM_Interface = {
    &DRA44XGEM_init,
    &DRA44XGEM_exit,
    &DRA44XGEM_start,
    &DRA44XGEM_stop,
...
DRA44XGEM_init (IN ProcessorId  dspId, IN DSP_Object * dspState)
{
    ...
    MemMapInfo          mapInfo ;
    DRA44XGEM_HalObj *  halObj  = NULL   ;
    LINKCFG_Dsp *       dspObj           ;
    LINKCFG_DspConfig * dspCfg ;
    LINKCFG_MemEntry *  memTable ;
    Uint32              i ;

    TRC_1ENTER ("DRA44XGEM_init", dspId) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        if (     (dspObj->doDspCtrl != DSP_BootMode_Boot_Pwr)
            ...
            if (DSP_SUCCEEDED (status)) {
                status = DRA44XGEM_halInit (&dspState->halObject, NULL) ;
                if (DSP_FAILED (status)) {
                    SET_FAILURE_REASON ;
                }
                else  {
                    halObj = (DRA44XGEM_HalObj *) dspState->halObject ;
                }
            }
    ...
    TRC_1LEAVE ("DRA44XGEM_init", status) ;
 ...
 *          2. Call DRA44XGEM_idle () to let the DSP run in 'self loop'.
...
DRA44XGEM_exit (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    LINKCFG_Dsp *       dspObj           ;
    LINKCFG_MemEntry *  memTable         ;
    DRA44XGEM_HalObj *  halObj           ;
    LINKCFG_DspConfig * dspCfg           ;
    Uint32              i                ;

    TRC_1ENTER ("DRA44XGEM_exit", dspId) ;
    ...
    if (IS_VALID_PROCID (dspId) == FALSE) {
        ...
        DRA44XGEM_halExit (&dspState->halObject) ;
    }

    TRC_1LEAVE ("DRA44XGEM_exit", status) ;
                 ...
                 IN Uint32       dspAddr)
{
    ...
    TRC_2ENTER ("DRA44XGEM_start", dspId, dspAddr) ;
    ...
    TRC_1LEAVE ("DRA44XGEM_start", status) ;
...
DRA44XGEM_stop (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("DRA44XGEM_stop", dspId, dspState) ;
    ...
    TRC_1LEAVE ("DRA44XGEM_stop", status) ;
...
DRA44XGEM_idle (IN ProcessorId dspId, IN DSP_Object * dspState)
{
    ...
    TRC_2ENTER ("DRA44XGEM_idle", dspId, dspState) ;
    ...
    TRC_1LEAVE ("DRA44XGEM_idle", status) ;
                   ...
                   IN OUT     Pvoid             arg)
{
    ...
    TRC_5ENTER ("DRA44XGEM_intCtrl", dspId, dspState, intId, cmd, arg) ;
    ...
    TRC_1LEAVE ("DRA44XGEM_intCtrl", status) ;
                 ...
                 OUT Uint8 *      buffer)
{
    ...
    TRC_1LEAVE ("DRA44XGEM_read", status) ;
                  ...
                  IN Uint8 *       buffer)
{
    ...
    TRC_1LEAVE ("DRA44XGEM_write", status) ;
                        ...
                        IN  DSP_AddrConvType   type)
{
    ...
    TRC_4ENTER ("DRA44XGEM_addrConvert", dspId, dspState, addr, type) ;
    ...
    TRC_2PRINT (TRC_LEVEL1,
                "DRA44XGEM_addrConvert addr [0x%x] convAddr [0x%x]\n",
                addr,
                convAddr) ;

    TRC_0LEAVE ("DRA44XGEM_addrConvert") ;
                        ...
                        Pvoid       arg)
{
    ...
    TRC_4ENTER ("DRA44XGEM_control", dspId, dspState, cmd, arg) ;

    DBC_Require (IS_VALID_PROCID (dspId)) ;

    status = DSP_ENOTIMPL ;

    TRC_1LEAVE ("DRA44XGEM_control", status) ;

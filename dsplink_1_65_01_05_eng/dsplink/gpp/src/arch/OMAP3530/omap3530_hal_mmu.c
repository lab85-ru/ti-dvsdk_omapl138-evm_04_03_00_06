...
/*  ============================================================================
 *  @const  OMAP3530_HalMmuEntriesCount,
 *
 *  @desc   TLB entry counter in MMU.Global
 *  ============================================================================
 */
STATIC Uint32 OMAP3530_HalMmuEntriesCount [4] ;
 ...
 *  ----------------------------------------------------------------------------
 */
STATIC List * OMAP3530_HalMmuEventList ;
 ...
 *  ============================================================================
 */
STATIC MMU_EntryInfo * OMAP3530_HalMmuEntryInfo ;
 ...
 *  ============================================================================
 */
STATIC Uint32 OMAP3530_HalMmuIrqStatus = 0u ;
STATIC Uint32 OMAP3530_HalMmuFaultAddr = 0u ;

/** ============================================================================
 *  @func   OMAP3530_halMmuISR
 *
...
NORMAL_API
DSP_STATUS
OMAP3530_halMmuISR (IN Pvoid refData)
{
    DSP_STATUS status    = DSP_SOK ;

    TRC_0ENTER ("OMAP3530_halMmuISR") ;
    ...
    PRINT_Printf ("DSP MMU Error Fault! "
                  ...
                  OMAP3530_HalMmuIrqStatus,
                  OMAP3530_HalMmuFaultAddr) ;

    OMAP3530_HalMmuIrqStatus = 0x0u ;
    OMAP3530_HalMmuFaultAddr = 0x0u ;

    TRC_1LEAVE ("OMAP3530_halMmuISR", status) ;
...
/** ============================================================================
 *  @func   OMAP3530_halMmuCheckIntGen
 *
...
EXPORT_API
Bool
OMAP3530_halMmuCheckIntGen (IN Pvoid refData)
{
    ...
    TRC_1ENTER ("OMAP3530_halMmuCheckIntGen", refData) ;

    halObject = (OMAP3530_HalObj *) refData ;

    /* Store the values for displaying later */
    OMAP3530_HalMmuIrqStatus = REG (halObject->mmuBase + MMU_IRQSTATUS_OFFSET) ;
    OMAP3530_HalMmuFaultAddr = REG (halObject->mmuBase + MMU_FAULT_AD_OFFSET) ;

    /* De-assert the interrupt */
    REG (halObject->mmuBase + MMU_IRQSTATUS_OFFSET) = OMAP3530_HalMmuIrqStatus ;

    /* Disable anymore from being generated. */
    REG (halObject->mmuBase + MMU_IRQENABLE_OFFSET) = 0u ;

    TRC_1LEAVE ("OMAP3530_halMmuCheckIntGen", isAsserted) ;
...
/** ============================================================================
 *  @func   OMAP3530_halSearchMmuEntry
 *
...
NORMAL_API
Bool
OMAP3530_halSearchMmuEntry (IN ListElement * elem, Pvoid data)
{
    MMU_EntryInfo * info          = NULL    ;
    MMU_EntryInfo * addInfo       = NULL    ;
    Bool                  retVal     = FALSE ;

    TRC_2ENTER ("OMAP3530_halSearchMmuEntry", elem, data) ;
    ...
    TRC_1LEAVE ("OMAP3530_halSearchMmuEntry", retVal) ;

    return retVal ;
}

/** ============================================================================
 *  @func   OMAP3530_halAddMmuEntry
 *
                 ...
                 IN Uint32 processSize)
{
    ...
    TRC_0ENTER ("OMAP3530_halMmuAddEntry") ;
    ...
    if (OMAP3530_HalMmuEventList == NULL) {
    ...
    status = LIST_SearchElement (
               (List *) OMAP3530_HalMmuEventList,
               (Pvoid) &addInfo,
               (ListElement **) &info,
               (ListMatchFunc) (&OMAP3530_halSearchMmuEntry)) ;

    if ((status == DSP_ENOTFOUND) || (status == DSP_EINVALIDARG)) {
        status = MEM_Alloc ((Void **) &OMAP3530_HalMmuEntryInfo,
        ...
        if (DSP_FAILED (status)) {
            ...
            status = LIST_InitializeElement (
                                         &(OMAP3530_HalMmuEntryInfo->element)) ;
            DBC_Assert (DSP_SUCCEEDED (status)) ;

            if (DSP_SUCCEEDED (status)) {
                OMAP3530_HalMmuEntryInfo->dspVirtAddr = dspVirtAddr ;
                OMAP3530_HalMmuEntryInfo->actualSize = actualSize ;
                OMAP3530_HalMmuEntryInfo->processAddr = processAddr ;
                OMAP3530_HalMmuEntryInfo->processSize = processSize ;
                OMAP3530_HalMmuEntryInfo->tlbEntry = tlbIndex ;
                OMAP3530_HalMmuEntryInfo->type = type ;

                status = LIST_PutTail (OMAP3530_HalMmuEventList,
                                       &(OMAP3530_HalMmuEntryInfo->element)) ;
                if (DSP_FAILED (status)) {
                    SET_FAILURE_REASON ;
                }
    ...
    TRC_1LEAVE ("OMAP3530_halMmuAddEntry", status) ;

    return status ;
}

/** ============================================================================
 *  @func   OMAP3530_halMmuRegion
 *
...
EXPORT_API
DSP_STATUS
OMAP3530_halMmuRegion(IN OMAP3530_HalObj * halObject,
                    IN Uint32      dspVirtAddr,
                    IN Uint32      size,
                    IN Bool         type
                     )
{
    ...
    TRC_0ENTER ("OMAP3530_halMmuRegion") ;
    ...
    if ((size & (SIZE_4KB - 1)) != 0) {
        ...
        PRINT_Printf ("Configuration error (A):"
                      ...
                      " Look-aside Buffers. ") ;
        if (   (OMAP3530_HalMmuEntriesCount [0] != 0)
            || (OMAP3530_HalMmuEntriesCount [1] != 0)
            || (OMAP3530_HalMmuEntriesCount [2] != 0)
            || (OMAP3530_HalMmuEntriesCount [3] != 0)) {
                PRINT_Printf ("The configured entries are:\n"
                              ...
                              OMAP3530_HalMmuEntriesCount [3],
                              OMAP3530_HalMmuEntriesCount [2],
                              OMAP3530_HalMmuEntriesCount [1],
                              OMAP3530_HalMmuEntriesCount [0]) ;
        }
        status = DSP_ECONFIG ;
        SET_FAILURE_REASON ;
    }
    ...
    while ((size != 0) && DSP_SUCCEEDED (status)) {
        ...
        if(DSP_SUCCEEDED (status)) {
            status = OMAP3530_halMmuAddEntry (halObject->tlbIndex,
                                           type,
                                           dspVirtAddr,
                                           actualSize,
                                           processAddr,
    ...
    TRC_1LEAVE ("OMAP3530_halMmuRegion", status) ;
...
/** ============================================================================
 *  @func   OMAP3530_halCheckMmuEntry
 *
 *  @desc   This function matches an event listener with a list element.
 *
 *  @modif  None.
...
OMAP3530_halCheckMmuEntry (IN ListElement * elem, Pvoid data)
{
    ...
    TRC_2ENTER ("OMAP3530_halCheckMmuEntry", elem, data) ;
    ...
    TRC_1LEAVE ("OMAP3530_halCheckMmuEntry", retVal) ;
...
NORMAL_API
DSP_STATUS
OMAP3530_halMmuEnable (IN  Pvoid              halObj,
                       ...
                       IN  Uint32             memEntries)
{
    ...
    TRC_3ENTER ("OMAP3530_halMmuEnable", halObj,memTable,memEntries) ;
    ...
    if (DSP_FAILED (status)) {
        ...
        for (i = 0 ; i < NUM_SIZE_ENTRIES_TLB; i++) {
            OMAP3530_HalMmuEntriesCount[i] = 0 ;
        }
        ...
        status = LIST_Create (&OMAP3530_HalMmuEventList) ;
        if (DSP_FAILED (status)) {
            SET_FAILURE_REASON ;
        }
        for (i = 0 ; i < sortedCnt && DSP_SUCCEEDED (status); i++) {
            /* Configure the TLB */
            if (sorted[i].size != 0) {
                status = OMAP3530_halMmuRegion(halObject,
    ...
    if (DSP_SUCCEEDED (status)) {
        ...
        intObj.shared = FALSE ;
        intObj.checkFunc = OMAP3530_halMmuCheckIntGen ;
        intObj.param     = halObject ;
        status = ISR_Create (OMAP3530_halMmuISR,
                             (Pvoid) halObject,
                             &intObj,
                             &(halObject->isrObject)) ;
        if (DSP_FAILED (status)) {
    ...
    TRC_1LEAVE ("OMAP3530_halMmuEnable", status) ;
...
NORMAL_API
DSP_STATUS
OMAP3530_halMmuDisable (IN         Pvoid          halObj)
{
    DSP_STATUS         status    = DSP_SOK   ;
    ...
    TRC_1ENTER ("OMAP3530_halMmuDisable", halObj) ;
    ...
    if ((OMAP3530_HalMmuEventList!= NULL) && (DSP_SUCCEEDED (tmpStatus))) {
        status = LIST_First (OMAP3530_HalMmuEventList,
                                     (ListElement **) &info) ;
        while ((info != NULL) && (DSP_SUCCEEDED (status))) {
            if (DSP_SUCCEEDED (status)) {
                status = LIST_Next (OMAP3530_HalMmuEventList,
                                       (ListElement *)  info,
                                       (ListElement **) &nextInfo) ;
                if (DSP_FAILED (status)) {
                    SET_FAILURE_REASON ;
                }
                status = LIST_RemoveElement (OMAP3530_HalMmuEventList,
                                              &(info->element)) ;
                if (DSP_FAILED (status)) {
                    SET_FAILURE_REASON ;
                }
        ...
        if (DSP_SUCCEEDED (status)) {
            status = LIST_Delete (OMAP3530_HalMmuEventList) ;;
            if (DSP_FAILED(status)) {
                SET_FAILURE_REASON ;
            }
        }
    ...
    TRC_1LEAVE ("OMAP3530_halMmuDisable", status) ;
...
/** ============================================================================
 *  @func   OMAP3530_halMmuDeleteEntry
 *
...
EXPORT_API
DSP_STATUS
OMAP3530_halMmuDeleteEntry(IN OMAP3530_HalObj * halObject,
                           ...
                           IN Bool              type)
{
    ...
    TRC_3ENTER ("OMAP3530_halMmuDeleteEntry", halObject, mmuInfo, type) ;
    ...
    while ((mmuInfo->size != 0) && DSP_SUCCEEDED (status)) {
        ...
        if (OMAP3530_HalMmuEventList!= NULL) {
            ...
            status = LIST_SearchElement (
                       ...
                       (Pvoid) &delInfo,
                       (ListElement **) &info,
                       (ListMatchFunc) (&OMAP3530_halCheckMmuEntry)) ;
        }
        if (DSP_SUCCEEDED (status)) {
            ...
            tmpStatus = LIST_RemoveElement (OMAP3530_HalMmuEventList,
                            &(info->element)) ;
    ...
    TRC_1LEAVE ("OMAP3530_halMmuDeleteEntry", status) ;
...
NORMAL_API
DSP_STATUS
OMAP3530_halMmuCtrl (IN         Pvoid               halObj,
                     ...
                     IN         ProcMemMapInfo *    mmuInfo)
{
    ...
    TRC_3ENTER ("OMAP3530_halMmuCtrl", halObj,memTable,memEntries) ;
    ...
    switch (cmd) {
        ...
        {
            status = OMAP3530_halMmuEnable ((Pvoid)halObj,
                                            memTable,
                                            memEntries) ;
            if (DSP_FAILED (status)) {
                SET_FAILURE_REASON ;
        ...
        {
            status = OMAP3530_halMmuDisable ((Pvoid)halObj) ;
            if (DSP_FAILED (status)) {
                SET_FAILURE_REASON ;
            }
        ...
        case DSP_MmuCtrlCmd_DeleteEntry:
        {
            ...
            status = OMAP3530_halMmuDeleteEntry (halObject,
                                                 mmuInfo,
                                                 type) ;
            if (DSP_FAILED (status)) {
                SET_FAILURE_REASON ;
    ...
    TRC_1LEAVE ("OMAP3530_halMmuCtrl", status) ;

...
XDAS_Int32 main( )
{
  ...
  XDAS_Int8 *fname = "..\\..\\Test\\TestVecs\\Config\\Testvecs_enc.cfg";
  ...
  { 
    ...
    printf("\nCreating Algorithm Instance...");
      ...
      if ((handle =  (ISPHENC1_Handle) ALG_create (
                                      (IALG_Fxns *) &G711ENC_TII_IG711ENC,
                                      (IALG_Handle) NULL,
    ...
    do
    {
      ...
      if(testCompliance)
      {
        ...
        if(testVal == XDM_EOK)
        {
          ...
          printf("\n Encoder compliance test passed for frame # %d",frameCount);
        }
        else 
        {
          /* Test Compliance Failed... Breaking...                         */
          printf("\n Encoder compliance test failed for frame # %d",frameCount);
          break;
        }
      }
      else
      {
        ...
        printf("\n Encoder output dump completed for frame # %d",frameCount);
      }
    } while(inBytesRead > 0);   /* end of Do-While loop                          */

    /* Destroy the handle instance */
    ALG_delete ((ISPHENC1_Handle)handle);
...
XDAS_Void byteSwapBitStream (XDAS_Int16 *p,
                              XDAS_Int16 size)
{
 XDAS_Int16 i;
 for(i=0; i<size; i++)

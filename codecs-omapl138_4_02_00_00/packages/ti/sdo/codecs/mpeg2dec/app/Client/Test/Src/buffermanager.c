...
#define NUMREFBUFS 3
XDAS_UInt32 alignBytes_sub[MAX_BUFF_ELEMENTS][NUMREFBUFS];
...
XDAS_Int32 BUFFMGR_Init(XDAS_Int32 numBufs, XDAS_Int32 *bufSizes)
{
    ...
    for(idx = 0; idx < numBufs; idx++)
    {
		tmpBufPtr =  malloc((XDAS_UInt32)bufSizes[idx]+128);
        alignBytes = (((XDAS_UInt32) tmpBufPtr + (128 - 1)) & ~ (128 - 1));
        alignBytes -= (XDAS_UInt32) tmpBufPtr;
		tmpBufPtr +=alignBytes;
        alignBytes_sub[0][idx] = alignBytes;
...
{
    XDAS_UInt8 *tmpBufPtr;
    XDAS_UInt32 idx, ctr,alignBytes;
    ...
    for(idx = 1; idx < numRefBufs; idx++)
    {
        ...
        {
           tmpBufPtr =  malloc((XDAS_UInt32)bufSizes[ctr]+128);
           alignBytes = (((XDAS_UInt32) tmpBufPtr + (128 - 1)) & ~ (128 - 1));
           alignBytes -= (XDAS_UInt32) tmpBufPtr;
		   tmpBufPtr +=alignBytes;
           alignBytes_sub[idx][ctr] = alignBytes;
...
)
{
    ...
    for(idx = 0; idx < numRefBufs; idx++)
    {
        for(ctr = 0; ctr < numOutBufs; ctr++)
        {
           buffArray[idx].buf[ctr] = (XDAS_UInt8 *)((XDAS_UInt32)(buffArray[idx].buf[ctr]) - alignBytes_sub[idx][ctr]); 

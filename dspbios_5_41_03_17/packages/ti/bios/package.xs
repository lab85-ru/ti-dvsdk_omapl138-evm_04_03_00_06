...
{
    /*
     *  Load RTDX package since BIOS apps need to link with RTDX.
     *  The BIOS-generated .cmd file contains rtdx library name, while the
     *  rtdx package provides linker path information to find this
     *  library.  This is not the right way to do if for full RTSC build
     *  but BIOS 5.x apps are still using legacy .cdb and generated .cmd file. 
     */
    xdc.loadPackage('ti.rtdx');

    xdc.loadPackage('ti.psl');

    xdc.loadPackage('ti.pmi');

    xdc.loadPackage('ti.pscl');
}

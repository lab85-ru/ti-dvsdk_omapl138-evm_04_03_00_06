...
static Int DOV_close(DEV_Handle device)
{
    ...
    while (!QUE_empty(copy->dobj.todevice)) {
        ...
        QUE_putJ(device->todevice, frame);
    }
    ...
    /* free overlap buffer */
    MEM_freeJ(0, copy->overlap, copy->size);

    /* recycle copy object */ 
    MEM_freeJ(0, copy, sizeof(DOV_CopyObj));
...
static Int DOV_open(DEV_Handle device, String name)
{
    ...
    if ((copy = MEM_allocJ(0, sizeof(DOV_CopyObj), 0)) == MEM_ILLEGAL) {
        return (SYS_EALLOC);
    }
    ...
    if ((copy->overlap = MEM_vallocJ(0, size, 0, DOV->INITIAL)) == MEM_ILLEGAL) {
        goto e1;
    }
    ...
    while (!QUE_empty(device->todevice)) {
        ...
        QUE_putJ(copy->dobj.todevice, frame);
    }
